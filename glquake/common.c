...
//
  search = Hunk_AllocName (sizeof(searchpath_t), "search");
...
//
  COM_AddGameDirectory (va("%s%c"GAMENAME, basedir, c) );

  if (COM_CheckParm ("-rogue"))
    COM_AddGameDirectory (va("%s%crogue", basedir, c) );
  if (COM_CheckParm ("-hipnotic"))
    COM_AddGameDirectory (va("%s%chipnotic", basedir, c) );
...
//
  ...
  {
    com_modified = true;
    COM_AddGameDirectory (va("%s%c%s", basedir, c, com_argv[i+1]));
  }
