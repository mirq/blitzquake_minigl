...
// fail if the command already exists
  ...
  cmd = Hunk_AllocName (sizeof(cmd_function_t), "cmdfunc");
  cmd->name = cmd_name;
  ...
  if ((!Q_strcasecmp(Cmd_Argv(0), "say") || !Q_strcasecmp(Cmd_Argv(0), "say_team")) && Cmd_Argc() > 1)
  {
    ...
    while (*src && dst - buff < 100)
    {
      if (*src == '%')
      {
        ...
        case 'r':
          if (cl.items & IT_ROCKET_LAUNCHER)
          {
            if (cl.stats[STAT_ROCKETS] < 5)
              dst += sprintf(dst, "I need rockets");
            else
              dst += sprintf(dst, "I have RL");
          }
          else
            dst += sprintf(dst, "I need RL");
          break;
