...
void Sbar_DeathmatchOverlay (void)
{
  ...
  // JPG - check to see if we need to update ping times
  if ((cl.last_ping_time < cl.time - 5) && !cl.sbar_ping && pq_scoreboard_pings.value)
  {
    cl.sbar_ping = true;
    Cbuf_AddText("ping\n");
  }
...
//  Draw_Pic (64, 24, pic);
  Draw_Pic (((vid.width - 320)/2)+64, 24, pic);


  pic = Draw_CachePic ("gfx/inter.lmp");
//  Draw_TransPic (0, 56, pic);
  Draw_TransPic (((vid.width - 320)/2), 56, pic);
...
//  Draw_TransPic (266,64,sb_nums[0][num%10]);
  Draw_TransPic (((vid.width - 320)/2)+234,64,sb_colon);
  Draw_TransPic (((vid.width - 320)/2)+246,64,sb_nums[0][num/10]);
  Draw_TransPic (((vid.width - 320)/2)+266,64,sb_nums[0][num%10]);

  Sbar_IntermissionNumber (((vid.width - 320)/2)+160, 104, cl.stats[STAT_SECRETS], 3, 0);

  Draw_TransPic (((vid.width - 320)/2)+232,104,sb_slash);
  Sbar_IntermissionNumber (((vid.width - 320)/2)+240, 104, cl.stats[STAT_TOTALSECRETS], 3, 0);

  Sbar_IntermissionNumber (((vid.width - 320)/2)+160, 144, cl.stats[STAT_MONSTERS], 3, 0);
  Draw_TransPic (((vid.width - 320)/2)+232,144,sb_slash);
  Sbar_IntermissionNumber (((vid.width - 320)/2)+240, 144, cl.stats[STAT_TOTALMONSTERS], 3, 0);
