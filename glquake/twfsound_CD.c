...
/*GFS*/  struct TWFCDData * TWFCD_Setup(STRPTR DeviceName, UBYTE UnitNum)
{
    ...
    // create the table...
    if(!(NewData = (struct TWFCDData *)Sys_Alloc(sizeof(struct TWFCDData),
                                                 MEMF_ANY|MEMF_CLEAR)))
      return(NULL);
    (NewData->TWFCD_Cmd).cdc_SCSIData = (UBYTE *)
                                        Sys_Alloc(CDBUFFER_SIZE,
    ...
    (NewData->TWFCD_Cmd).cdc_SCSISense = (UBYTE *)
                                         Sys_Alloc(CDSENSE_SIZE,
                                                   MEMF_CHIP|MEMF_CLEAR);
    if (!((NewData->TWFCD_Cmd).cdc_SCSISense))
    {
      Sys_Free((NewData->TWFCD_Cmd).cdc_SCSIData);
      Sys_Free(NewData);
      return 0;
    }
    (NewData->TWFCD_Cmd).cdc_TOCBuffer = (UBYTE *)
                                         Sys_Alloc(CDTOC_SIZE,
                                                   MEMF_CHIP|MEMF_CLEAR);
    if (!((NewData->TWFCD_Cmd).cdc_TOCBuffer))
    {
      Sys_Free((NewData->TWFCD_Cmd).cdc_SCSIData);
      Sys_Free((NewData->TWFCD_Cmd).cdc_SCSISense);
      Sys_Free(NewData);
    }
