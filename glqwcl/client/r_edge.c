...
#ifdef PROXY
extern cvar_t r_fastcontents;
extern void D_DrawSurfaces_FlatContents(void);
#endif
...
{
  int   iv, bottom;
  static byte  basespans[MAXSPANS*sizeof(espan_t)+CACHE_SIZE]; /*phx*/
  espan_t *basespan_p;
  surf_t  *s;
...
#ifdef PROXY
        {
        if(!r_fastcontents.value)
#endif
        D_DrawSurfaces ();
#ifdef PROXY
        else
        D_DrawSurfaces_FlatContents();
        }
#endif
