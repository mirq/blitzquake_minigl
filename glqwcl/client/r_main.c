...
/*int   screenwidth; @@@ already defined in d_edge.c */
...
// FIXME: remove and do real lighting
  ...
  for (i=0 ; i<cl_numvisedicts ; i++)
  {
    ...
    // trivial accept status
      if (R_AliasCheckBBox ())
      {
	if((currententity->model)->modhint == MOD_FLAME || currententity->model)->modhint == MOD_FLAME2)
	  j = 256;
	else
...
void R_EdgeDrawing (void)
{
  static edge_t  ledges[NUMSTACKEDGES +
        ((CACHE_SIZE - 1) / sizeof(edge_t)) + 1]; /*phx*/
  static surf_t  lsurfs[NUMSTACKSURFACES +
        ((CACHE_SIZE - 1) / sizeof(surf_t)) + 1]; /*phx*/
...
{
#ifndef AMIGA
  /* Disabled for Amiga-port, which allocated a warpbuffer during */
  /* vid_amiga.c (phx) */
...
//surgeon: disable error checking for speed
/*  
