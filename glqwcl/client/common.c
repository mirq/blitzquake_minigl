...
//#include <ctype.h> //uncommented by surgeon
...
#ifdef __GNUC__ //broken updir in fopen() ?
	if(filename[0] == '/')
	{
	   int cut;

	   sprintf(netpath, "%s", search->filename);
	   cut = strlen(netpath);
	   ...
	   netpath[cut] = 0;
         ...
         sprintf (netpath, "%s%s", netpath, filename);
	}
	else
#endif
...
#ifdef AMIGA
  else if ((p = strrchr(dir, ':')) != NULL)  /*phx*/
    strcpy(gamedirfile, ++p);
#endif
  else
    strcpy(gamedirfile, dir);  /*fixed, phx*/
...
void COM_Gamedir (char *dir)
{
  ...
  sprintf (com_gamedir, "%s%s", com_basedir, dir);  /*phx*/
...
// (phx) Make sure that basedir path is terminated by '/' or ':'
  i = strlen(com_basedir);
  if (com_basedir[i-1]!='/' && com_basedir[i-1]!=':'
      && com_basedir[i-1]!='\\') {
    com_basedir[i] = '/';
    com_basedir[i+1] = 0;
  }
...
//
  COM_AddGameDirectory (va("%sid1", com_basedir) ); /*phx*/
  COM_AddGameDirectory (va("%sqw", com_basedir) ); /*phx*/
