...
cvar_t sbar_detail = {"sbar_detail","2"};
...
#if 0
extern void Draw_ArrayPic (int x, int y, qpic_t *pic, GLuint num);
extern void Draw_ArraySubPic (int x, int y, qpic_t *pic, int srcx, int srcy, int width, int height, GLuint num);


extern GLuint D_GenArray();
extern void D_CallArray(qpic_t *pic, GLuint num);
extern qboolean d_arraycache[256];

void Sbar_DrawAPic (int x, int y, qpic_t *pic, GLuint num)
{
  if(d_arraycache[num] == false)
  {
	Draw_ArrayPic (x, y + (vid.height-SBAR_HEIGHT), pic, num);
  }
  else
  	D_CallArray(pic, num);
}

void Sbar_DrawASubPic(int x, int y, qpic_t *pic, int srcx, int srcy, int width, int height, GLuint num) 
{
  if(d_arraycache[num] == false)
  {
	Draw_ArraySubPic (x, y+(vid.height-SBAR_HEIGHT), pic, srcx, srcy, width, height, num);
  }
  else
  	D_CallArray(pic, num);
}
...
#if 1
extern void Sbar_DrawHUDIbar(int x, int y);
#else
extern void Sbar_DrawHUDIbar(float x, float y);
#endif
...
/*
#define Sbar_DrawString(x, y, str) Draw_String((int)x, (int)y+ vid.height-SBAR_HEIGHT, (char *)str, 0)
*/

void Sbar_DrawString (int x, int y, char *str)
{
  Draw_String (x, y+ vid.height-SBAR_HEIGHT, str,0);
}
...
#if 1
	if (headsup)
	Sbar_DrawHUDIbar((hudswap) ? 0 : (vid.width-42), -24 - 44+(vid.height-SBAR_HEIGHT));
#endif
...
//surgeon: speedup
    ...
    Draw_Fill (x*8 + 10, y, 28, yt+yb, top);
...
void Sbar_MiniDeathmatchOverlay (void)
{
  ...
  char      name2[3+1]; //trunchated names speedup
  int	      board_detail; //surgeon: speedup

  if (vid.width < 512 || !sb_lines)
    return; // not enuff room

  board_detail = sbar_detail.value;

  if(!board_detail && cl_sbar.value)
	return;
...
// draw the text
  ...
  if(board_detail != 2 && scr_viewsize.value != 110)
	y += 16;
...
if(top != bottom) //speedup
{
    Draw_Fill ( x, y+1, 40, 3, top);
    Draw_Fill ( x, y+4, 40, 4, bottom);
}
else
    Draw_Fill ( x, y+1, 40, 7, top);
...
//surgeon: this is better
  ...
  if (board_detail == 2)
  {
   ...
   else if (board_detail == 1)
   {
	name2[3] = 0;
	strncpy(name2, s->name, 3);
	if (teamplay)
		Draw_String (x+48+40, y, name2,0);
	else
		Draw_String (x+48, y, name2,0);
   }

    y += 8;

//Surgeon: if there is room then draw bigger scoreboard
	if(vid.width >= 768 && x != 512)
	{
		if(y >= (vid.height - 8 + 1)) //bottom
		{	
			y  = (vid.height - sb_lines - 1);
