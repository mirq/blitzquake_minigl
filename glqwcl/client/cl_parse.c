/*
fpd and TF recognition implemented to prevent cheats
*/
...
#include "quakedef.h"
#ifdef PROXY
#include "teamplay.h" //Surgeon & Tonik
#endif

#ifdef PROXY
int fpd, fpdinit;	//surgeon - for proxyfeatures disable
int fortress;		//surgeon - for proxyfeatures disable
...
void Get_FPD (void)
{
 ...
 if (!fpdinit)
 {
 tfgame = (Info_ValueForKey(cl.serverinfo, "*gamedir"));
 fpd = atoi(Info_ValueForKey(cl.serverinfo, "fpd"));

	if(!strcmp(tfgame, "fortress")) //force fpd for TF
	{
		fortress = 1; //prevent bodyfilter
		if(!(fpd&256))
		fpd = fpd + 256;
		if(!(fpd&512))
		fpd = fpd + 512;
  ...
  if(fpd)
  Con_Printf("qwcl: server disables (fpd=%i):\n",fpd);
  else
  Con_Printf("qwcl: no features disabled\n");

	if(fpd&1)    Con_Printf("qwcl: - all % reporting\n");
	if(fpd&2)    Con_Printf("qwcl: - poweruptimers\n");
	if(fpd&4)    Con_Printf("qwcl: - soundtrigger\n");
	if(fpd&8)    Con_Printf("qwcl: - (lagfeature)\n");
	if(fpd&16)   Con_Printf("qwcl: - (report lagchange)\n");
	if(fpd&32)   Con_Printf("qwcl: - %e reporting\n");
	if(fpd&64)   Con_Printf("qwcl: - (spectator/player talk)\n");
	if(fpd&128)  Con_Printf("qwcl: - hide pointing (%txyjk)\n");
	if(fpd&256)  Con_Printf("qwcl: - skin forcing\n");
	if(fpd&512)  Con_Printf("qwcl: - color forcing\n");
...
Con_DPrintf ("signon request..\n");
  MSG_WriteByte (&cls.netchan.message, clc_stringcmd);
//  MSG_WriteString (&cls.netchan.message, va("prespawn %i 0 %i", cl.servercount, cl.worldmodel->checksum2));

  /* Bug fix! Server expects checksum in little endian format! (phx) */
  MSG_WriteString (&cls.netchan.message, va(prespawn_name, cl.servercount,
                   (unsigned)LittleLong(cl.worldmodel->checksum2)));
}
...
#ifdef PROXY
fpdinit = 0; //surgeon - reset fpd & fortress at levelchange
fortress = 0;
#endif
...
#else
if (cl.model_precache[es.modelindex]->modhint == MOD_FLAME || cl.model_precache[es.modelindex]->modhint == MOD_FLAME2)
#endif
...
ok: 
	...
	if (!strncmp(buf, " f_version\n", 11) || !strncmp(buf, " z_version\n", 11)) 
		Cbuf_AddText ("say Amiga-ZQuake version 0.10\n"); 
} 
