...
//surgeon start
	// remove dead bodies
	//(deathframes = 42-103 in player.mdl)
	if (s1->flags & PF_DEAD)
	{
		if (bodyfilter.value)
		{
		if (s1->frame > 41 && s1->frame < 104)
		continue;
		}
	}
//surgeon end
...
// surgeon start - remove gibs
if (gibfilter.value)
{
if (model->flags & (EF_GIB | EF_ZOMGIB))
ent->model = 0;
}
// surgeon end

//surgeon start - rocket to grenade model
	if(r2g.value)
	{
	if (model->flags & EF_ROCKET)
	 {
	 ent->model = Mod_ForName ("progs/grenade.mdl", true);
	 }
	}
//surgeon end
...
#ifdef PROXY
//surgeon start - frameskipping routines

if (frameskip.value)
{
	if (frameskip.value == 1) //every 5th frame
	{
	//set bodyfilter so deathframes doesn't need to be checked
	if(!bodyfilter.value)	Cvar_Set("bodyfilter","1");

	if (state->frame >= 42 && state->frame <= 103) continue; //dead
	if (!(state->frame % 5)) continue;
	}
	else if (frameskip.value == 2) //every 3rd frame
	{
	//set bodyfilter so deathframes doesn't need to be checked
	if(!bodyfilter.value)	Cvar_Set("bodyfilter","1");

	if (state->frame >= 42 && state->frame <= 103) continue; //dead
	if (!(state->frame % 3)) continue;
	}
      else //every 2nd frame
	{
	//set bodyfilter so deathframes doesn't need to be checked
	if(!bodyfilter.value)	Cvar_Set("bodyfilter","1");

	if (state->frame >= 42 && state->frame <= 103) continue; //dead
	if (state->frame & 1) continue;
	}
}
//surgeon end
#endif
