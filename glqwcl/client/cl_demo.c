...
#ifdef PROXY
#include "teamplay.h" //functions for easyrecord
#endif
...
char tmpdemo[MAX_OSPATH] = "ram:qwd/#.qwd";
char destdemo[MAX_OSPATH] = "qwd/#.qwd";
char com_bufferdir[MAX_OSPATH] = "ram:qwd";
char com_qwddir[MAX_OSPATH] = "qwd";

void COM_CopyDemoFile (char *frompath, char *destpath)
{
FILE  *indemo, *outdemo;
int   remaining, count;
char  buf[4096];

remaining = COM_FileOpenRead (frompath, &indemo);

indemo = fopen(frompath, "rb");
...
COM_CreatePath (destpath);
outdemo = fopen(destpath, "wb");
...
while (remaining)
      ...
      fread (buf, 1, count, indemo);
      fwrite (buf, 1, count, outdemo);
      remaining -= count;
      }
fclose(indemo);
fclose(outdemo);
Con_Printf("relocation complete\n");
}

void CL_CheckBufferRecord (void)
{
	if(COM_CheckParm("-bufrecord"))
	{
	Con_Printf("writing %s\n", destdemo);
	COM_CopyDemoFile(tmpdemo, destdemo);
	}
}

/*
void CL_RelocDemo_f (void)
{
	if(COM_CheckParm("-bufrecord"))
		COM_CopyDemoFile(tmpdemo, destdemo);
	else
		Con_Printf("-bufrecord not enabled\n");
...
#ifndef PROXY //fix demoplayback freeze but still corrupt :(
...
//surgeon start
  ...
  {
  Sys_mkdir("ram:qwd");
  sprintf(name, "%s/%s", com_bufferdir, Cmd_Argv(1));
  sprintf(tmpdemo, "%s/%s.qwd", com_bufferdir, Cmd_Argv(1));
  sprintf(destdemo, "%s/%s.qwd", com_qwddir, Cmd_Argv(1)); 
  }
  else
//surgeon end
#endif  
...
{ 
	...
	unsigned char	*p; 
	FILE	*f; 
	char	com_buffer_dir[MAX_OSPATH] = "ram:"; //surgeon	
...
/// FIXME: check buffer sizes!!! 
	...
	if (c == 2)
		...
		{ 
			// Teamplay 
			sprintf (name, "%s_%s_vs_%s_%s", CL_PlayerName(), CL_PlayerTeam(), CL_EnemyTeam(), CL_MapName()); 
		} else { 
			if (i == 2) { 
				// Duel 
			sprintf (name, "%s_vs_%s_%s", CL_PlayerName(), CL_EnemyName(), CL_MapName()); 
			} 
			else if (i > 2) { 
				// FFA 
			sprintf (name, "%s_ffa_%s", CL_PlayerName(), CL_MapName()); 
			} 
			else { 
				// one player 
			sprintf (name, "%s_%s", CL_PlayerName(), CL_MapName()); 
			} 
		} 
	} 
...
// Make sure the filename doesn't contain illegal characters 
	...
	{
	Sys_mkdir("ram:qwd");
	strncpy (name, va("%s/%s", com_qwddir, name), MAX_OSPATH-1);
	}
	else
...
if(COM_CheckParm("-bufrecord"))
//buffered recording by surgeon
	{
	strncpy (destdemo, va("%s", name2), MAX_OSPATH-1);
	strncpy (tmpdemo, va("%s%s", com_buffer_dir, name2), MAX_OSPATH-1);
	strncpy (name2, va("%s%s", com_buffer_dir, name2), MAX_OSPATH-1);
