...
//#define	MYSINCOSSCALE 2.8444444	//360->1024
#define	MYSINCOSSCALE 1.4222222	//360->512

qboolean mytabinit = false;

void MY_SINCOS_Init(void)
{
	int i;
	float temp;
	...
	for (i=0; i<512; i++)
	{
		temp = (float)(i/256.f*M_PI);
		mysintab[i] = (float)sin(temp);
	}

	mytabinit = true;
} 

float MYSIN(float an)
{
	int ang;
	if (an < 0.f)
	{
	ang = (an*MYSINCOSSCALE-0.5);
		return -(mysintab[(-ang)&511]);
	}
	ang = (an*MYSINCOSSCALE+0.5);
	return (mysintab[ang&511]);
}

float MYCOS(float an)
{
	int ang;
	if (an < 0.f)
	{
...
float MYTAN(float an)
{
	return (MYSIN(an)/MYCOS(an));
}

void MYSINCOS_Test_f (void)
{
float testval, sintest, costest;
  testval = Q_atof(Cmd_Argv(1)); 
  sintest = MYSIN(testval);
  costest = MYCOS(testval);

  Con_Printf("sin %f  cos %f\n", sintest, costest);
}
...
#else
  zrot[0][0] = MYCOS(degrees);
  zrot[0][1] = MYSIN(degrees);
  zrot[1][0] = -MYSIN(degrees);
  zrot[1][1] = MYCOS(degrees);
#endif
...
#else 
	//Surgeon - further optimization note: dir, point and degrees are targets for inline optimizations - dst (out frustum) changes between calls in a frame due to different combinations/ and +-
...
void RotatePointAroundVectorNEW( vec3_t dst, vec3_t dst2, const vec3_t dir, const vec3_t point, float degrees)
{

//added dst2 as parameter
//translate half the frustun normals in one go
	...
	dst2[0] = (t02 * vr[0] + t12 * vu[0] + vf[0] * vf[0]) * point[0] 
	       + (t02 * vr[1] + t12 * vu[1] + vf[0] * vf[1]) * point[1] 
	       + (t02 * vr[2] + t12 * vu[2] + vf[0] * vf[2]) * point[2]; 
	...
	dst2[1] = (t02 * vr[0] + t12 * vu[0] + vf[1] * vf[0]) * point[0] 
	       + (t02 * vr[1] + t12 * vu[1] + vf[1] * vf[1]) * point[1] 
	       + (t02 * vr[2] + t12 * vu[2] + vf[1] * vf[2]) * point[2]; 
	...
	dst2[2] = (t02 * vr[0] + t12 * vu[0] + vf[2] * vf[0]) * point[0] 
	       + (t02 * vr[1] + t12 * vu[1] + vf[2] * vf[1]) * point[1] 
	       + (t02 * vr[2] + t12 * vu[2] + vf[2] * vf[2]) * point[2]; 
}
...
#if !defined(PPCASM) && !defined(M68KASM) && !defined(GLPPCASM) && !defined(GLM68KASM)
...
{ 
	...
	{ 
	case 0: // x axis 
		p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSideX; 
		break; 
	case 1: // y axis 
		p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSideY; 
		break; 
	case 2: // z axis 
		p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSideZ; 
		break; 
	default: 
		if (p->normal[2] < 0) // 4 
		{ 
			...
			{ 
				if (p->normal[0] < 0) // 1 
					p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSide7; 
				else 
					p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSide6; 
			} 
			else 
			{ 
				if (p->normal[0] < 0) // 1 
					p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSide5; 
				else 
					p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSide4; 
			} 
		} 
		else 
		{ 
			...
			{ 
				if (p->normal[0] < 0) // 1 
					p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSide3; 
				else 
					p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSide2; 
			} 
			else 
			{ 
				if (p->normal[0] < 0) // 1 
					p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSide1; 
				else 
					p->BoxOnPlaneSideFunc = (BOPSfunc)BoxOnPlaneSide0; 
			} 
		} 
		break; 
	} 
...
*/
#ifdef BOPSDEBUG
int BoxOnPlaneSide2 (vec3_t emins, vec3_t emaxs, mplane_t *p)
#else
...
#if !defined(GLPPCASM) && !defined(GLM68KASM) && !defined(PPCASM) && !defined(M68KASM)
...
#endif /* !PPCASM/!M68KASM */


#if !defined(M68KASM) && !defined(GLM68KASM)
...
#if !id386 && !defined(M68KASM) && !defined(GLM68KASM)
