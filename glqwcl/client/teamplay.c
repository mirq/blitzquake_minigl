/* 
	...
	$Id: teamplay.c,v 1.14 2000/10/28 17:13:47 tonik Exp $ 
*/ 
...
char *Cmd_Macro_Weapon_f (void) // weap in hand and ammo
{ 
	...
	case IT_SHOTGUN:  
		sprintf(macro_buf, "sg:%i", cl.stats[STAT_SHELLS]); 
		return macro_buf; 
 
	case IT_NAILGUN:
		sprintf(macro_buf, "ng:%i", cl.stats[STAT_NAILS]); 
		return macro_buf; 

	case IT_SUPER_SHOTGUN:  
		sprintf(macro_buf, "ssg:%i", cl.stats[STAT_SHELLS]); 
		return macro_buf; 

	case IT_SUPER_NAILGUN: 
		sprintf(macro_buf, "sng:%i", cl.stats[STAT_NAILS]); 
		return macro_buf; 
 
	case IT_GRENADE_LAUNCHER:
		sprintf(macro_buf, "gl:%i", cl.stats[STAT_ROCKETS]);
		return macro_buf; 
 
	case IT_ROCKET_LAUNCHER: 
		sprintf(macro_buf, "rl:%i", cl.stats[STAT_ROCKETS]); 
		return macro_buf; 
 
	case IT_LIGHTNING: 
		sprintf(macro_buf, "lg:%i", cl.stats[STAT_CELLS]); 
		return macro_buf; 
...
static int _Cmd_Macro_BestWeapon (void) //disregard ammo
{ 
	int	best; 
...
// surgeon -->
char *Cmd_Macro_BestWeapon_f (void) //best weapon and ammo
{ 
...
static int _Cmd_Macro_NeedHealth (void) 
{ 
static int nhealth;
...
static int _Cmd_Macro_NeedArmor (void) 
{ 
static int narmor;
	...
	{
	if (cl.stats[STAT_ITEMS] & IT_ARMOR1 && cl.stats[STAT_ARMOR] < 75)
		narmor = 1;

	else if (cl.stats[STAT_ITEMS] & IT_ARMOR2 && cl.stats[STAT_ARMOR] < 50) 
		narmor = 1;
 
	else if (cl.stats[STAT_ITEMS] & IT_ARMOR3 && cl.stats[STAT_ARMOR] < 25) 
		narmor = 1; 
	else 
		narmor = 0;	// no armor at all
	}
return narmor;
}

static int	_Cmd_Macro_NeedWeapon (void) 
{ 
static int	wtype;

	wtype = 0;

	if (cl.stats[STAT_ITEMS] & IT_ROCKET_LAUNCHER || cl.stats[STAT_ITEMS] & IT_LIGHTNING || cl.stats[STAT_ITEMS] & IT_GRENADE_LAUNCHER || cl.stats[STAT_ITEMS] & IT_SUPER_SHOTGUN || cl.stats[STAT_ITEMS] & IT_SUPER_NAILGUN)
	wtype = 0; //have high damage weapon

	else 
	wtype = 1; //need high damage weapon
...
char	*n_shells(void) 
{ 
	if ((cl.stats[STAT_ITEMS] & IT_SUPER_SHOTGUN || cl.stats[STAT_ITEMS] & IT_SHOTGUN) && cl.stats[STAT_SHELLS] < 10)
	return "[s]";
	else
	return "";
}
...
char *n_nails(void)
{
	if ((cl.stats[STAT_ITEMS] & IT_NAILGUN || cl.stats[STAT_ITEMS] & IT_SUPER_NAILGUN) && cl.stats[STAT_NAILS] < 40)
	return "[n]";
	else
	return "";
}
...
char *n_rocks(void)
{
	if ((cl.stats[STAT_ITEMS] & IT_ROCKET_LAUNCHER || cl.stats[STAT_ITEMS] & IT_GRENADE_LAUNCHER) && cl.stats[STAT_ROCKETS] < 5)
	return "[r]";
	else
	return "";
}
...
char *n_cells(void)
{
	if (cl.stats[STAT_ITEMS] & IT_LIGHTNING && cl.stats[STAT_CELLS] < 20)
	return "[c]";
	else
	return "";
}
...
char *Cmd_Macro_Need_f (void) 
{ 
	switch (_Cmd_Macro_NeedHealth()) 
	{
	case 1:
		switch (_Cmd_Macro_NeedArmor())
		{		
		case 1:
			if(_Cmd_Macro_NeedWeapon())
			{
			sprintf(macro_buf, "H:A:W %s%s%s%s", n_shells(), n_nails(), n_rocks(), n_cells());
			return macro_buf;
			} 
 			else
			{
			sprintf(macro_buf, "H:A %s%s%s%s", n_shells(), n_nails(), n_rocks(), n_cells());
			return macro_buf;
			}
 		case 0:
			if(_Cmd_Macro_NeedWeapon())
			{
			sprintf(macro_buf, "H:W %s%s%s%s", n_shells(), n_nails(), n_rocks(), n_cells());
			return macro_buf;
			} 
			else
			{
			sprintf(macro_buf, "H %s%s%s%s", n_shells(), n_nails(), n_rocks(), n_cells());
			return macro_buf;
			}
	...
	case 0:
		...
		case 1:
			if(_Cmd_Macro_NeedWeapon())
			{
			sprintf(macro_buf, "A:W %s%s%s%s", n_shells(), n_nails(), n_rocks(), n_cells());
			return macro_buf;
			} 
 			else
			{
			sprintf(macro_buf, "A %s%s%s%s", n_shells(), n_nails(), n_rocks(), n_cells());
			return macro_buf;
			}
 		case 0:
			if(_Cmd_Macro_NeedWeapon())
			{
			sprintf(macro_buf, "W %s%s%s%s", n_shells(), n_nails(), n_rocks(), n_cells());
			return macro_buf;
			} 
			else
			{
			sprintf(macro_buf, "%s%s%s%s", n_shells(), n_nails(), n_rocks(), n_cells());
			return macro_buf;
			}
		default:
			return "";
	...
	{ 
		...
		{ 
			...
			{ 
				...
				case 'b': macro_string = Cmd_Macro_BestWeapon_f(); break; 
				...
				case 'u': macro_string = Cmd_Macro_Need_f(); break; 
...
{ 
	...
	Cmd_AddMacro("need", Cmd_Macro_Need_f);
} 
...
{ 
	...
	Cmd_AddCommand ("teamcolor", CL_TeamColor_f); 
	Cmd_AddCommand ("enemycolor", CL_EnemyColor_f); 
} 
#endif
