/*
odd correction and demoplayback "fix"
*/
...
#ifdef GLQUAKE
extern void Sbar_DrawHUDIbar(int x, int y, qpic_t *pic, int srcx, int srcy, int width, int height);
#endif
...
/*
//macro with proper typecasting by surgeon
#define Sbar_DrawSubPic(x, y, pic, srcx, srcy, width, height){\
Draw_SubPic(x, y+(vid.height-SBAR_HEIGHT), pic, srcx, srcy, width, height);}
*/
...
//surgeon start
	  if (odd_display)
/*
          Sbar_DrawSubPic ((hudswap) ? 0 : (vid.width-24),-18-(7-i)*8 , sb_weapons[flashon][i],0,0,24,16);
*/
          Sbar_DrawPic ((hudswap) ? 0 : (vid.width-24),-18-(7-i)*8 , sb_weapons[flashon][i]);
...
*/
       Sbar_DrawPic ((hudswap) ? 0 : (vid.width-24),-68-(7-i)*16 , sb_weapons[flashon][i]);
	  }
      }
	else
	{ 
...
//surgeon start
       ...
       Sbar_DrawPic (i*24, 4, sb_weapons[flashon][i]);
//surgeon end
...
//surgeon end
	...
	if (headsup)
	Sbar_DrawHUDIbar((hudswap) ? 0 : (vid.width-42), -24 - 44+(vid.height-SBAR_HEIGHT), sb_ibar, 3, 0, 42, 11);
#endif
...
else
{
  for (i=0 ; i<4 ; i++)
  {
    ...
    if (num[0] != ' ')
      Sbar_DrawCharacter ( (6*i+1)*8 - 2, -1, 18 + num[0] - '0');
    if (num[1] != ' ')
      Sbar_DrawCharacter ( (6*i+2)*8 - 2, -1, 18 + num[1] - '0');
    if (num[2] != ' ')
      Sbar_DrawCharacter ( (6*i+3)*8 - 2, -1, 18 + num[2] - '0');
...
//surgeon start
        ...
        Sbar_DrawPic (192 + i*16, 4, sb_items[i]);    
//surgeon end
...
//surgeon start
        ...
        Sbar_DrawPic (320-32 + i*8, 4, sb_sigil[i]); 
//surgeon end
...
//    Draw_Fill (xofs + x*8 + 10, y+4, 28, 3, bottom);
    Draw_Fill (x*8 + 10, y, 28, yt, top);
    Draw_Fill (x*8 + 10, y+yt, 28, yb, bottom);
//surgeon end
...
if (odd_display)
   {    
    Sbar_DrawCharacter ( (x+1)*8 , -1, num[0]);
    Sbar_DrawCharacter ( (x+2)*8 , -1, num[1]);
    Sbar_DrawCharacter ( (x+3)*8 , -1, num[2]);

    if (k == cl.playernum)
    {
      Sbar_DrawCharacter (x*8+2, -1, 16);
      Sbar_DrawCharacter ( (x+4)*8-4, -1, 17);
    }
   }
else
   {    
...
//surgeon end
  ...
  {
    Sbar_DrawPic (112, y, sb_face_invis_invuln);
    return;
  }
  if (cl.stats[STAT_ITEMS] & IT_QUAD) 
  {
    Sbar_DrawPic (112, y, sb_face_quad );
    return;
  }
  if (cl.stats[STAT_ITEMS] & IT_INVISIBILITY) 
  {
    Sbar_DrawPic (112, y, sb_face_invis );
    return;
  }
  if (cl.stats[STAT_ITEMS] & IT_INVULNERABILITY) 
  {
    Sbar_DrawPic (112, y, sb_face_invuln);
    return;
  }
  ...
  if (cl.time <= cl.faceanimtime)
  {
    ...
    anim = 0;
  Sbar_DrawPic (112, y, sb_faces[f][anim]);
}
...
//surgeon end
  ...
  Sbar_DrawPic (0, y, sb_sbar);

// armor
  if (cl.stats[STAT_ITEMS] & IT_INVULNERABILITY)
  {
    Sbar_DrawNum (24, y, 666, 3, 1);
    Sbar_DrawPic (0, y, draw_disc);
  }

  else
  {
    Sbar_DrawNum (24, y, cl.stats[STAT_ARMOR], 3
    , cl.stats[STAT_ARMOR] <= 25);
    if (cl.stats[STAT_ITEMS] & IT_ARMOR3)
      Sbar_DrawPic (0, y, sb_armor[2]);
    else if (cl.stats[STAT_ITEMS] & IT_ARMOR2)
      Sbar_DrawPic (0, y, sb_armor[1]);
    else if (cl.stats[STAT_ITEMS] & IT_ARMOR1)
      Sbar_DrawPic (0, y, sb_armor[0]);
  }
...
// health
  Sbar_DrawNum (136, y, cl.stats[STAT_HEALTH], 3
  , cl.stats[STAT_HEALTH] <= 25);

// ammo icon

  if (cl.stats[STAT_ITEMS] & IT_SHELLS)
    Sbar_DrawPic (224, y, sb_ammo[0]);
  else if (cl.stats[STAT_ITEMS] & IT_NAILS)
    Sbar_DrawPic (224, y, sb_ammo[1]);
  else if (cl.stats[STAT_ITEMS] & IT_ROCKETS)
    Sbar_DrawPic (224, y, sb_ammo[2]);
  else if (cl.stats[STAT_ITEMS] & IT_CELLS)
    Sbar_DrawPic (224, y, sb_ammo[3]);
  
  Sbar_DrawNum (248, y, cl.stats[STAT_AMMO], 3
...
if (odd_display)
{
  if (sb_lines > 12 || (scr_viewsize.value <= 100))
  {
    if (!cl.spectator || autocam == CAM_TRACK || cls.demoplayback)
      Sbar_DrawInventory ();
    if (!headsup)
      Sbar_DrawFrags ();
  } 
...
// main area
  if (sb_lines > 0)
  {
//surgeon: added "!cls.demoplayback" as condition
...
//surgeon start
	  ...
	  {
        Sbar_DrawPic (0, 12, sb_scorebar);
        Sbar_DrawString (8,14, "SPEC MODE - Press [ATTACK] for AutoCam");
	  }
//surgeon end
...
//  Draw_Pic (160-pic->width/2, 0, pic);
  Draw_Pic ((vid.width/2)-pic->width/2, 0, pic);
...
x = ((vid.width - 320)/2) + 36;
  ...
  Draw_String(x, y, "low/avg/high team total players",0);
...
//surgeon end
  ...
  y += 8;
//  Draw_String(x, y, "------------ ---- ----- -------",0);
  Draw_String(x, y, "\x1d\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1f \x1d\x1e\x1e\x1f \x1d\x1e\x1e\x1e\x1f \x1d\x1e\x1e\x1e\x1e\x1e\x1f",0);
...
// draw the text
  ...
  // draw pings
    ...
    Draw_String ( x, y, num,0);

  // draw team
    team[4] = 0;
    strncpy (team, tm->team, 4);
    Draw_String (x + 104, y, team,0);

  // draw total
    sprintf (num, "%5i", tm->frags);
    Draw_String (x + 104 + 40, y, num,0);
    
  // draw players
    sprintf (num, "%5i", tm->players);
    Draw_String (x + 104 + 88, y, num,0);
...
//surgeon start - added odd_display as contition
  if (start || odd_display)
//surgeon end
...
//                            0    40 64   104   152  192 
    Draw_String ( x , y, "ping pl time frags team name",0);
    y += 8;
//    Draw_String ( x , y, "---- -- ---- ----- ---- ----------------");
    Draw_String ( x , y, "\x1d\x1e\x1e\x1f \x1d\x1f \x1d\x1e\x1e\x1f \x1d\x1e\x1e\x1e\x1f \x1d\x1e\x1e\x1f \x1d\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1f",0);
...
//                            0    40 64   104   152
    Draw_String ( x , y, "ping pl time frags name",0);
    y += 8;
//    Draw_String ( x , y, "---- -- ---- ----- ----------------",0);
    Draw_String ( x , y, "\x1d\x1e\x1e\x1f \x1d\x1f \x1d\x1e\x1e\x1f \x1d\x1e\x1e\x1e\x1f \x1d\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1f",0);
...
//surgeon end
  ...
  for (i=0 ; i<l && y <= vid.height-10 ; i++)
  {
    ...
    // draw pl
if (!s->spectator) //surgeon : don't draw pl for spectators!
    {
    p = s->pl;
    sprintf (num, "%3i", p);
    if (p > 25)
      ...
      Draw_String ( x+32, y, num,0);
    }
else
    {
      Draw_String (x+40, y, "(spectator)",0);
      // draw name
      if (teamplay)
        Draw_String (x+152+40, y, s->name,0);
      else
        Draw_String (x+152, y, s->name,0);
      y += skip;
      continue;
    }
...
//surgeon start  
 ...
 { 
   Draw_Fill (x+104, y+2, 40, 2, top);
   Draw_Fill (x+104, y+4, 40, 3, bottom);
 }
//surgeon end  
...
// draw the text
  ...
  // draw name
    ...
    if (teamplay)
      Draw_String (x+48+40, y, name,0);
    else
      Draw_String (x+48, y, name,0);
    y += 8;
  }
  ...
  // draw pings
    team[4] = 0;
    strncpy (team, tm->team, 4);
    Draw_String (x, y, team,0);

  // draw total
    sprintf (num, "%5i", tm->frags);
    Draw_String (x + 40, y, num,0);
