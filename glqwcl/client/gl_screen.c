...
#ifdef PROXY
// --> Tonik 
cvar_t		scr_clock = {"clock","0"}; 
cvar_t		scr_clock_x = {"clock_x","0"}; 
cvar_t		scr_clock_y = {"clock_y","-1"}; 
...
//Surgeon: adjust for aspect
r_refdef.fov_y = CalcFov (r_refdef.fov_x, r_refdef.vrect.width*vid.aspect, r_refdef.vrect.height);
...
//
#ifdef AMIGA
  Cmd_AddCommand ("mglshot",SCR_MGLSHOT_f);
#endif
...
//surgeon start
extern int FirstTime; // fix zquake code 
//surgeon end
...
//surgeon start 
	if (scr_clock.value == 2)  // time of day 
	      time = FirstTime + cl.time;
	else
		time = cl.time;
		hours = fmod (time / 3600, 24); 
//surgeon end
	...
	sprintf (str, "%i:%i%i:%i%i", hours, tens_minutes, minutes,  
		tens_seconds, seconds); 
	if (scr_clock_y.value < 0) 
		Draw_String (8 * scr_clock_x.value, vid.height - sb_lines + 8*scr_clock_y.value, str,0); 
	else 
		Draw_String (8 * scr_clock_x.value, 8*scr_clock_y.value, str,0); 
} 
// <-- Tonik 
#endif
...
// 
  ...
  { 
    ppmname[5] = i/10 + '0'; 
    ppmname[6] = i%10 + '0'; 
    sprintf (checkname, "%s/%s", com_gamedir, ppmname);
    if (Sys_FileTime(checkname) == -1)
      break;  // file doesn't exist
  } 
  if (i==100) 
  {
    Con_Printf ("SCR_MGLSHOT_f: Couldn't create a PPM file\n"); 
    return;
  }
