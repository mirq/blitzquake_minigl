...
#include "cl_slist.h"
#include "teamplay.h"
extern void CL_EasyRecord_f (void);
...
extern int fpd;	//surgeon

cvar_t	frameskip = {"frameskip","0"};
cvar_t	bodyfilter = {"bodyfilter","0"};
cvar_t	gibfilter = {"gibfilter","0"};
cvar_t	beamskip = {"beamskip","0"};

#ifndef GLQUAKE
cvar_t	r_dynamic = {"r_dynamic","1"};
#endif

cvar_t	r_explosions = {"r_explosions","1"};
cvar_t	r_teleports = {"r_teleports","1"};
cvar_t	v_alert = {"v_alert","1"};
cvar_t	v_powerup = {"v_powerup","1"};
cvar_t	r_flames = {"r_flames","1"};
cvar_t	r2g = {"r2g","0"};
cvar_t	cl_muzzleflash = {"cl_muzzleflash","1"};
cvar_t	r_fastcontents = {"r_fastcontents","0"};
cvar_t	r_skycolor = {"r_skycolor","215"};
cvar_t	r_turbcolor = {"r_turbcolor","185"};
...
void CL_ForceTeamColor_f (void)
{
	...
	if (!strcmp(Cmd_Argv(1), "off") || (fpd&512))
	{
		cl_teamtopcolor = -1;
		return;
	}
...
*/
	...
	Cvar_RegisterVariable (&r_explosions);
	Cvar_RegisterVariable (&r_teleports);
	Cvar_RegisterVariable (&frameskip);
	Cvar_RegisterVariable (&bodyfilter);
	Cvar_RegisterVariable (&gibfilter);
	Cvar_RegisterVariable (&beamskip);
	Cvar_RegisterVariable (&v_alert);
	Cvar_RegisterVariable (&v_powerup);
	Cvar_RegisterVariable (&r_flames);
	Cvar_RegisterVariable (&r2g);
	Cvar_RegisterVariable (&cl_muzzleflash);
	Cvar_RegisterVariable (&r_fastcontents);
...
#ifdef PROXY
//quakeforge:
	if ((serlist = fopen (va ("%s/servers.txt", com_gamedir), "r"))) { 
		Con_Printf("CL_Init:found servers.txt\n");
		slist = SL_LoadF (serlist, slist); 
		if(serlist) fclose (serlist); 
	} 
#endif
...
int error; //cdaudio glquake bug
  ...
  Con_Printf ("GLQwcl%s V%4.2f\n",
              ...
              AMIGA_VERSION);
	#ifdef GLQUAKE
  Con_Printf ("Amiga port 2001 by Christian Michael\n");
	#else
  Con_Printf ("Amiga port 2000 by Frank Wille and Steffen Haeuser\n");
	#endif
#endif
//  Con_Printf ("Exe: "__TIME__" "__DATE__"\n");
...
//  S_Init ();    // S_Init is now done as part of VID. Sigh.
  ...
  Cbuf_AddText ("echo Type connect <internet address> or use\n");
  Cbuf_AddText ("echo GameSpy to connect to a game.\n");
...
extern void R_FreeParticles(void);
extern void qgl_FreeArrays(void);
