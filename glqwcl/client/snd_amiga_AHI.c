...
qboolean SNDDMA_Init (void)
{
  ...
  if (i = COM_CheckParm("-audspeed")) {
    def_smp_speed = (UWORD)Q_atoi(com_argv[i+1]);
  }
  else
    def_smp_speed = (UWORD)(desired_speed);
...
//    shm->speed = def_smp_speed;

    shm->samplebits = 16;
//    shm->samples = AHIBUFFERSIZE / (shm->samplebits / 8);
    shm->samples = AHIBUFFERSIZE / 2;
    ...
    *ahi_channel_info[1].AHIio = *ahi_channel_info[0].AHIio;
    ...
    ahi_channel_info[0].starttime = Sys_DoubleTime ();
    ...
    start_ahi_sound (&ahi_channel_info[1], shm->buffer, AHIBUFFERSIZE, ahi_channel_info[0].AHIio);
  ...
  if (using_ahi) {
   ...
   else
   {
          ...
          CheckIO ((struct IORequest *)c->AHIio))
	{
        ...
        c->sound_in_progress = FALSE;
/*
        if ((now = Sys_DoubleTime ()) > (c->starttime =
        c2->starttime + ahi_playtime))
*/
	  c->starttime = c2->starttime + ahi_playtime;
	  now = Sys_DoubleTime ();
        if (now > c->starttime)
        c->starttime = now;

        start_ahi_sound (c, shm->buffer, AHIBUFFERSIZE, ahi_channel_info[1 - which_buffer].AHIio);
        which_buffer = 1 - which_buffer;

        c = &ahi_channel_info[which_buffer];
      }

      shm->samplepos = ((int)((now + ahi_playtime - c->starttime) * twice_real_speed + 0.5)) & (AHIBUFFERSIZE - 1);
   }
    ...
    if (!channel_info[0].sound_in_progress)
      ...
      shm->samplepos = ((int)((Sys_DoubleTime() - channel_info[0].starttime)
