...
void VID_Init_CGFX (unsigned char *palette)
{
  ...
  {
   CYBRMREQ_MinWidth, 320, CYBRMREQ_MinHeight, 120, CYBRMREQ_MaxWidth, 1280, CYBRMREQ_MaxHeight, 1024, CYBRMREQ_CModelArray,(ULONG)ColorModel, TAG_DONE,0
  };
...
//surgeon end
  ...
  else
  {
    ...
    for (i=0;i<8;i++)
      ...
      Sys_Error("%sCan't allocate temporary bitmap  (raster)",module);
  }
...
void VID_ExpandBuffer(int width, int height, int realwidth, byte *vid_buffer)
{
	byte *tmp_buffer;
	int in_bufsize;
	int i,j;
	...
	out_bufsize = (realwidth*(height-sb_lines));
	in_bufsize = (width*(height-sb_lines));
	bufheight = height - sb_lines;

//double width scaling

do {
	for(j=0; j<in_bufsize; j+=width)
	{
		stepheight = k*width;
	
		for(i=j ;i<stepheight; i++)
		{
		tmp_buffer[i] = vid_buffer[i];
		tmp_buffer[i+1] = vid_buffer[i];
		}
	}
	k++;	
    } while(k!=bufheight); 
...
#else
    if (!turbogfx)
	ScalePixelArray(vid_buffer, 320, 50, BytesPerRow, &(QuakeScreen->RastPort), 0, 0,  320, 200, RECTFMT_LUT8); 
//      WritePixelArray8(&(QuakeScreen->RastPort),rects->x,rects->y, xstop,ystop,vid_buffer,&TempRP);
