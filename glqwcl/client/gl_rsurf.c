...
#include "miniglext.h" //vertexbuffer speedup, fake multitex

#ifdef AMIGA
cvar_t gl_fake_multitexture = {"gl_fake_multitexture","1", true};
extern cvar_t r_vertexarrays;
extern cvar_t r_particle_hack;
#endif

extern int R_VisibleCullBox (vec3_t mins, vec3_t maxs);
...
#ifdef TRIGTABLES
#define MYRADTODEG 57.295777951 
extern float MYSIN (float an);
#endif
...
store:
 ...
 case MGL_UNSIGNED_SHORT_5_6_5: //by Surgeon
    ...
    for (i=0 ; i<tmax ; i++, dest += stride)
    {
	  ...
	  short_dest[0] = ( (UWORD)(((t & 0xf8) << 8) | ((t & 0xfc) << 3) | ((t & 0xf8) >> 3)) );
 	  dest += 2;
      }
    }
  break;
...
//Surgeon:
//this looks cleaner, but it is faster to have them inline
void DrawGLWaterPolyMultiTexture (glpoly_t *p);
void DrawGLPoly (glpoly_t *p);
void DrawGLPolyLightmap (glpoly_t *p);
void DrawGLPolyMultiTexture (glpoly_t *p);
...
// Note: vertexarray approach turned out to slow things down
// Vertexarrays currently used for models and particles
...
static void R_DrawMultitextureBuffer()
{
	GLenum src, dst;
...
void R_BlendLightmaps (void)
{
  ...
  fake_multitexture = gl_fake_multitexture.value;
...
#ifndef AMESA
    glEnable (MGL_Z_OFFSET); //virge fix?
#endif
}
...
#ifndef AMESA
  glDisable (MGL_Z_OFFSET); //surgeon: virge fix?
#endif
...
//Surgeon: mirror always disabled - no need to check
    /*
...
void R_DrawBrushModel (entity_t *e)
{
  ...
  if(!gl_texsort.value)
  {
	if(gl_lightmap_format == GL_ALPHA)
		glBlendFunc(GL_ZERO, GL_SRC_ALPHA);
	else if(gl_lightmap_format == GL_LUMINANCE || gl_lightmap_format == MGL_UNSIGNED_SHORT_5_6_5)
		glBlendFunc(GL_ZERO, GL_SRC_COLOR);
  }
...
e->angles[0] = -e->angles[0]; // stupid quake bug
  ...
  if(!gl_texsort.value)
  {
	if(gl_lightmap_format != MGL_UNSIGNED_SHORT_4_4_4_4 && gl_lightmap_format != GL_RGBA)
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
  }
  else
  {
...
//          continue;   // wrong side
        ...
        if (/* !(((r_viewleaf->contents==CONTENTS_EMPTY && (surf->flags & SURF_UNDERWATER) && r_dowarp) ||
          (r_viewleaf->contents!=CONTENTS_EMPTY && !(surf->flags & (r_dowarp ? SURF_UNDERWATER : 0))))
...
//#endif
  ...
  for (i=0 ; i<MAX_LIGHTMAPS ; i++)
  {
    ...
    glTexImage2D (GL_TEXTURE_2D, 0, gl_lightmap_format, BLOCK_WIDTH, BLOCK_HEIGHT, 0, gl_lightmap_format, GL_UNSIGNED_BYTE,  lightmaps+i*BLOCK_WIDTH*BLOCK_HEIGHT*lightmap_bytes);
